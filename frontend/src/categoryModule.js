...
const categoryModule = 
{
    ...
    mutations: {
        ...
        updateCategory(state, {id, name, alias}){
            const categoryToUpdate = state.categorys.find(category => category.id == id);
            if (categoryToUpdate) {
        ...
        deleteCategory(state, id) {
            const indexToDelete = state.categorys.findIndex(category => category.id === id);
            if (indexToDelete !== -1) {
              // 선택한 id의 카테고리 삭제
    ...
    actions: {
        ...
        async addCategory(context, newCategory) {
            try {
                // 서버에 카테고리 추가 요청 보내기
                await axios.post(HOST+'/category/add', { categoryId: newCategory.id, categoryName: newCategory.name });
        ...
        async deleteCategory(context, id) {
            try {
                // 서버에 카테고리 삭제 요청 보내기
                await axios.post(HOST+'/category/delete', { categoryId: id });
        ...
        async updateCategory(context, { id, name, alias}) {
            try {
                ...
                await axios.post(HOST+'/category/update', { categoryId: id, categoryName: name/*, newAlias */});
        ...
        async applyCategory(context, newCategorys) {
            ...
            try {
                ...
                await axios.post(HOST+'/category/apply', newCategorys);
        ...
        async cancelCategory(context) {
            try {
                // 서버에 카테고리 취소 요청 보내기
                await axios.post(HOST+'/category/cancel');
