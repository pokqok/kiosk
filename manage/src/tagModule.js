...
const tagModule = 
{
    ...
    actions: {
        ...
        async addTag(context, newTag) {
            try {
                // 서버에 카테고리 추가 요청 보내기
                await axios.post(HOST+'/tag/addTag', { TagId: newTag.id, TagName: newTag.name, TagAlias: newTag.alias, TagDescription: newTag.description });
        ...
        async deleteTag(context, id) {
            try {
                // 서버에 카테고리 삭제 요청 보내기
                alert('id: ' + id);
                await axios.post(HOST+'/tag/deleteTag', { TagId: id });
        ...
        async updateTag(context, { id, name, alias, description}) {
            try {
                ...
                alert('이름: ' + name + 'id: ' + id);
                await axios.post(HOST+'/tag/updateTag', { TagId: id, TagName: name, TagAlias: alias, TagDescrption: description});
        ...
        async cancelTag(context) {
            try {
                // 서버에 카테고리 취소 요청 보내기
                await axios.post(HOST+'/tag/cancel');
