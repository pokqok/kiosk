<template>
    <v-subheader class = "subheader">옵션(태그) 관리
    ...
    <v-table>
      ...
      <tbody>
        <tr v-for="item in Tags" :key="item.name">
          ...
          <td> <v-switch v-model="item.isOn" color="green" @change="isOnSwitch(item.id, item.isOn)"></v-switch> </td>
          <td> <v-switch v-model="item.isRequired" color="green" @change="requireSwitch(item.id, item.isRequired)"></v-switch> </td>
          <td> 옵션버튼 추가 예정 </td>
        </tr>
      </tbody>
    </v-table>
    ...
    <v-dialog v-model="addTagDialog" persistent max-width="600px">
        <v-card>
          <v-card-title style="display: flex; justify-content: space-between;">
            태그 추가하기
            <v-btn variant="plain" density="compact" icon="mdi-close" @click="addTagDialog = false"></v-btn>
          </v-card-title>

          <v-container class="my-5">
            <v-form>
              <div>
                <v-text-field v-model="newTagName" class="pd-3"  density="compact" clearable label="태그 이름" variant="outlined" append>
                </v-text-field>

                <v-text-field v-model="newAliasName" class="pd-3"  density="compact" clearable label="태그 별칭" variant="outlined" append>
                </v-text-field>

                <v-textarea v-model="newDescription" class="pd-3"  density="compact" clearable label="태그 설명" variant="outlined" append>
                </v-textarea>
              </div>
              <v-btn @click="addTag">
                    추가하기
    ...
    <!--태그의 옵션 관리창 -->
      <v-dialog v-model="editTagDialog" persistent max-width="600px">
        <v-card>
          <v-card-title style="display: flex; justify-content: space-between;">
            <div>카테고리 설정</div>
                    <v-btn variant="plain" density="compact" icon="mdi-close" @click="closeEditTag()"></v-btn>
          </v-card-title>
          <v-card-text>
            <v-form @submit.prevent="applyEditTag">
              <v-text-field v-model="editedTagName" class="pd-3"  density="compact" clearable label="태그 이름" variant="outlined" append>
              </v-text-field>
              <v-text-field v-model="editedTagAlias" class="pd-3"  density="compact" clearable label="태그 별칭" variant="outlined" append>
              </v-text-field>
              <v-textarea v-model="editedTagDescription" class="pd-3"  density="compact" clearable label="태그 설명" variant="outlined" append>
              </v-textarea>

              <div style="display: flex; justify-content: space-between;">
                <div></div>
                <v-btn color="primary" type="submit" @click="updateTag(selectedTagId)">적용</v-btn>
    ...
    export default {
      ...
      */
      ...
      methods: {
        ...
        addTag(){//현재 form 태그 추가하기
          if (this.newTagName.trim() !== '' && this.newAliasName.trim() !== '') {
            const newTag = { id: this.Tags.length + 1, name: this.newTagName, alias: this.newAliasName, description: this.newDescription, isOn: true ,isRequired: false}; // 예시 데이터
        ...
        updateTag(selectedTagId){
          if (this.editedTagName.trim() !== '') {
            ...
            //alert(editedId);
            this.$store.dispatch('updateTag', { id: editedId, name: newName, alias: newAlias, description: newDescription});
        ...
        isOnSwitch(editedId, changedState){//표시여부스위치 활성화
          console.log(`Tag ID ${editedId}의 스위치 상태가 변경되었습니다. 새로운 상태: ${!changedState}`);
          console.log(editedId)
          this.$store.dispatch('toggleIsOn', { id: editedId , isOn: changedState} );
        },

        requireSwitch(editedId, changedState){ //필수여부 스위치 활성화
          console.log(`Tag ID ${editedId}의 스위치 상태가 변경되었습니다. 새로운 상태: ${!changedState}`);
          console.log(editedId)
          this.$store.dispatch('toggleRequired', { id: editedId , isRequired: changedState} );
        },
